...
load C:\LAURA\kemans_23anos\COPERNICUS\K\EKE_coper.mat
%whos; 
...
for irow = 1 : rows
    for icol = 1 : columns
        ...
        m_pcolor(xx,yy,reshape(modos(it,:),nx,ny));
        %colormap(map)
        colormap(map)
        hold on 
        ...
        ax = gca;
        clim = round(1000*mean(abs(quantile(modos(:), [0.05,0.95]))))/1000;
          caxis([0.02, 0.25]);
        if (irow == rows) && (icol == 1)
...
save('data_eke_somcoper.mat', 'evolution_best_match', 'modos');
