...
% Calcular la energía cinética en cada punto y tiempo
EKE = 0.5 * (up.^2 + vp.^2); % up y vp son (nlat*nlon x nt)
EKE = EKE';

a_EKE = find(any(isnan(EKE), 2)); % Índices con NaNs en u
b_EKE = find(~any(isnan(EKE), 2)); % Índices válidos en u
% Remover filas con NaNs por separado
EKE(a_EKE, :) = [];

tiene_nan = any(isnan(EKE(:)));
disp(['EKE contiene NaNs: ', num2str(tiene_nan)]);

mke = EKE; % Transponer nuevamente para formato SOM (características x muestras)
...
mapa = zeros(nx*ny,1);
mapa(a_EKE) = NaN;
mapa(b_EKE) = evolution_best_match;
...
geotiffwrite('mapa_resultado_tiff_VORT.tif', mapa_2d, R);